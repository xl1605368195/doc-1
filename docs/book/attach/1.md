...
Attach API的包名称为`com.sun.tools.attach`。包含2个类：VirtualMachine和VirtualMachineDescriptor。如下图3-1所示。

> 图3-1 Attach API 官方文档

![图3-1 Attach API 官方文档](../../.vuepress/public/images/book/attach/3-1.png)

VirtualMachine代表一个Java虚拟机，也就是监控的目标虚拟机，而VirtualMachineDescriptor用来描述虚拟机信息，配合VirtualMachine类实现各种功能。

主要的功能实现在`VirtualMachine`以及子类中，其它类起到辅助作用。下面的代码使用Attach API连接到PID为72695的JVM进程上，然后读取目标JVM的系统参数并输出到终端，最后调用detach方法与目标JVM断开连接。
...
```
上面代码第9行处，可以直观的理解在调用attach方法之后，就获得了一个目标JVM的VirtualMachine对象，调用VirtualMachine对象的方法（第12行处调用getSystemProperties方法）就可以完成对目标JVM的操作。
...
除了获取目标JVM系统参数的方法之外，VirtualMachine还有其他重要的方法，如下所示。
```text
// 列出当前主机上的所有JVM
