...
有了前面一节的使用基础，我们将分析attach api的实现原理并对相应的源码做解析，从而挖掘更多可用的功能。
`com.sun.tools.attach.VirtualMachine`是抽象类，
不同厂商的虚拟机可以实现不同VirtualMachine，
...
为了更加清晰的看到通信协议，这里使用strace跟踪attach的系统调用过程。
...
以Linux系统为例子，说明目标JVM如何处理attach请求和解析attach命令。

先来看下目标JVM如何处理`kill -3`信号。JVM初始化过程中会创建2个线程，
线程名称分别为`Signal Dispatcher`和`Attach Listener`，
Signal Dispatcher线程用来处理信号量，Attach Listener线程用来处理attach操作。
...
initialize_jdk_signal_support的实现代码如下
...
```
从上面的宏定义可以看出，SIGBREAK信号就是SIGQUIT。 
...
如果AttachListener没有初始化，
则判断临时目录下.attach_pid<pid>文件是否存在，
如果存在则调用init初始化AttachListener，初始化成功后返回true。

### Attach Listener
Attach机制通过Attach Listener线程来进行相关事务的处理，下面来看一下Attach Listener线程是如何初始化的。
从上面的代码分析可以看出，AttachListener可以在JVM启动时初始化，也可以在首次收到SIGBREAK信号时初始化。
...
重点分析下attach_listener_thread_entry函数。
```text
...
```
第一步先执行AttachListener socket的初始化操作，初始化完成后设置
AttachListener的状态为initialized。
...
```
不同操作系统执行初始化逻辑不同，在linux系统中实际执行LinuxAttachListener::init
```text
...
```
不难发现，AttachListener::pd_init()方法又调用了LinuxAttachListener::init()方法，完成了套接字的创建和监听。
...
本文对attach机制的全流程代码做了完整的分析，包括客户端如何发起attach，服务端如何响应attach命令。
