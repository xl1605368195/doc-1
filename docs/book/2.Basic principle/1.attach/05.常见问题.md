...
## 1.不同JDK版本的attach成功后返回结果不同

### 现象

当使用JDK11去attach jdk8 应用时，会抛异常com.sun.tools.attach.AgentLoadException: 0 ，
但实际上已经attach成功了。
...
在jdk11返回的是"return code: 0" 表示attach成功
...
发起attach的进程兼容不同版本jdk返回结果。下面是arthas对这个问题的兼容性处理:
...
代码位置：arthas/core/src/main/java/com/taobao/arthas/core/Arthas.java#L122
...
并且/tmp目录下没有attach通讯的socket文件进程.java_pid${pid}文件
...
文件/tmp目录下（不同操作系统tmp目录位置不同），该目录不可以被参数修改。
这个文件首次被创建后，JVM会标记attach listener 为initialized，
...
这个fix在增加了INITIALIZING、NOT_INITIALIZED、INITIALIZED几种状态，并且在INITIALIZED状态下通过AttachListener::check_socket_file进行自检
如果发现文件不存在，会清理之前的listener，并重新建立
...
```
是否有价值：
https://www.cnblogs.com/perfma/p/13043554.html


相关参考：

https://mail.openjdk.org/pipermail/serviceability-dev/2019-June/028499.html

https://bugs.openjdk.org/browse/JDK-8227815

## 3.attach进程的权限问题

### 现象

如果在root用户下执行jstack，而目标JVM进程不是root权限，执行报错如下：
...
## 6.com.sun.tools.attach.AttachNotSupportedException: no providers installed
...
https://blog.csdn.net/JavaMBa/article/details/121108753
