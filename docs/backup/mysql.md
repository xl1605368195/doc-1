...
[使用官方yum仓库](https://dev.mysql.com/downloads/repo/yum/)

![img.png](docs/.vuepress/public/images/guide/install/mysql/img.png)

官网下载链接

![img_1.png](docs/.vuepress/public/images/guide/install/mysql/img_1.png)
...
![img_2.png](docs/.vuepress/public/images/guide/install/mysql/img_2.png)
...
```
如果报错：mysql-community-client-5.7.39-1.el7.x86_64.rpm 的公钥尚未安装
![img_3.png](docs/.vuepress/public/images/guide/install/mysql/img_3.png)
...
```
![img_4.png](docs/.vuepress/public/images/guide/install/mysql/img_4.png)
### 执行安装
```json
...
```
![img.png](docs/.vuepress/public/images/guide/install/read-password.png)
...
### 尝试连接并修改默认密码
```json
mysql -uroot -p'EdnjfBqYh5<O'
```
修改为自己的密码
```shell
alter user root@localhost identified by 'tA@feH8W2NKNsMxx';
## 刷新权限
FLUSH PRIVILEGES;
```
### 远程连接
```shell
# 允许本地用户链接
mysql> grant all privileges on *.* to root@localhost identified by 'tA@feH8W2NKNsMxx';
 
Query OK, 0 rows affected, 1 warning (0.00 sec)

# 允许任意IP连接
mysql> grant all privileges on *.* to root@'%' identified by 'tA@feH8W2NKNsMxx';

Query OK, 0 rows affected, 1 warning (0.00 sec)

```
### 表结构初始化
这里使用 mysql workbench(免费客户端) 操作 (2022-10-03)
```shell
